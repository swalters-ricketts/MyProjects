...
GitHub Public URL:
https://github.com/Shaunb1998/NetworkNinjas-POS-ITT103-SP2026
https://github.com/swalters-ricketts/MyProjects

Purpose of the program:
This program is designed to create a simulated point of sale system used in a retail environment by a cashier. The system assists the cashier in processing customers’ transactions efficiently and reduces manual labour for stock-taking.
User Input Validation
The program requires the cashier to enter a customer's name, which helps the cashier to keep track of customers' orders and adds a touch of personal care to the service being offered. This also allows the program to be versatile as it can be used in different environments. If invalid inputs are entered, the cashier is prompted to try again, which allows a smaller margin for human error.
...
If checks are valid, the item/s selected are added to the cart, and stock is updated accordingly 
...
The program allows the cashier to process multiple customers in one session. After completing a transaction, the system asks whether to process another customer and allows the cashier to input a new customer's name. This allows the company to easily track how many people the cashier served during a shift and enhances check-off and accountability for each employee.
